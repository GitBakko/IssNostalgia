...
- **Physics tick**: 120 Hz
- **Substep policy**: adaptive 4 / 6 / 8 based on `|v|` (`<15`, `[15,25)`, `≥25`)
- **Spin vector ω**: world space

### Non-Negotiable Constraints
1. `RigidBody3D.custom_integrator = true` for the ball — always
2. Drag is quadratic — never linear
3. Magnus always active. Zero spin = zero Magnus, not "Magnus disabled"
4. No tunneling — `continuous_cd = true` + adaptive substeps + swept-sphere backup
...
without a new rationale entry. Consult `PHYSICS_LOG.md` for the live list — it
...
- After each task: commit → push → wait for explicit "ok prosegui" before next task (Sprint 01 only; later sprints may switch to auto)
