...
| Item | Status | Notes |
|------|--------|-------|
| Lob second-bounce "schizzo" | **CLOSED 2026-05-12 (S05-A02 supersedes A01)** | First-pass close (S05-A01) decided the artefact was purely perceptual based on the T01 energy harness PASS — and the harness is still correct (no net energy is created). But a literature review (Cross 2002 / 2010 / 2014, Carré et al. 2004 *Sports Engineering* 7:113) showed our Cross-2002 implementation was missing a **tangential dissipation term that does NOT scale with `e_n`**: on a soccer ball on grass the horizontal velocity post-bounce is measured at 55–75 % of the pre-bounce value even at grazing angles, while our model preserved ≥ 90 %. S05-A02 adds (i) a Cross-2014 surface-compliance tangential restitution `e_t_surface` applied in parallel with the Coulomb impulse, and (ii) an explicit retention clamp `|v_t_out| ≤ lerp(floor, ceil, angle_factor) · |v_t_in|`. The "schizzo" should now be visibly damped because the third-bounce horizontal speed is forced down to the measured envelope instead of being conserved by the collapsing Coulomb branch |
...
| ID      | Decision | Rationale |
|---------|----------|-----------|
| S05-A01 | LMB lob "schizzo" is perceptual, not a physics bug — **no parameter change**, close PENDING | T01 `tests/unit/test_bounce_energy.gd` simulates four shot shapes (spinless lob, topspin curve, backspin drop, strong-topspin slip-case stressor) through `integrate_step_pure` + `resolve_static_collisions` with drag / grass / Magnus / knuckle disabled, and asserts `KE_post(i+1) ≤ KE_post(i) · (1 + 1e-2)` between consecutive hard bounces. All 4 cases pass — the model does not create energy. T06 frame-step replay (P / `,` / `.`) let the user walk the bounce sequence tick-by-tick on the actual lob shape; user confirmation: "non si tratta di un vero aumento di velocità — il terzo rimbalzo perde molta parabola, la velocità residua viene proiettata in avanti molto di più del rimbalzo precedente, dando l'effetto schizzo". Mechanism: angle-aware restitution (S02-A10) collapses `e_eff` toward zero on grazing impacts → the bounce arc shrinks fast across consecutive hits → horizontal speed (preserved by the slip case) drives the ball forward visibly faster than the previous, much taller arc did. **Decision superseded by S05-A02** — closing as perceptual was wrong: a literature review showed the slip-case Coulomb-only friction is incomplete for a soccer ball on grass |
| S05-A02 | Add Cross-2014 surface-compliance tangential restitution + retention clamp to `_bounce_cross_2002` | User pushback after S05-A01: "non mi pare molto fisico che il terzo rimbalzo perda in parabola ma acquisti in lunghezza, mai visto un comportamento così di una palla da calcio su un campo in erba". A researcher pass over Cross 2002 (AJP 70:1093), Cross 2010 (AJP 78:1015), Cross 2014 (Experimental Mechanics 54:1523), Carré et al. 2004 (Sports Engineering 7:113), and the Rocket League GDC 2018 talk surfaced the gap: our slip-case friction is `μ_s · J_n = μ_s · (1+e_n) · m · \|v_n\|`. When the angle-aware `e_n` collapses on grazing impacts, J_n vanishes and so does the friction — the horizontal speed survives nearly intact. Real measurements on soccer balls on grass (Carré 2004) report 55–75 % horizontal retention even at low angles, because grass + envelope absorb tangential energy independently of how much normal energy was returned. Fix: (i) compute a second tangential impulse `J_t_surface = (1+e_t_surface) · \|v_c\| · m · k/(1+k)` with `e_t_surface = 0.25` (Cross 2014, grass range 0.20–0.35), and take `J_t_mag = clamp(max(J_t_coulomb, J_t_surface), 0, J_t_grip)` — so dissipation never falls below the surface-compliance floor. (ii) Add a final retention clamp `\|v_t_out\| ≤ lerp(floor, ceil, angle_factor) · \|v_t_in\|` with floor=0.60, ceil=0.85 derived from Carré 2004 envelope. The earlier `test_cross_topspin_keeps_or_gains_forward_speed` assertion `≥ 95 %` no longer holds and was rewritten to "topspin retains MORE than no-spin" (qualitative claim, still load-bearing). All four T01 energy tests still PASS — dissipation went up, conservation still holds |
| S05-A03 | **Lock knuckleball parameters (SUPERSEDED by S05-A04)** — measured drift 1.159 m / 1.5 s at 28 m/s @ 10° is inside the target arcade band [0.80, 1.50 m] | Standard "tiro 4" knuckle launch was already inside the desired arcade-feeling envelope (Carré 2004 reports ~0.3–0.6 m real-world drift at this speed; we want exaggerated arcade values ~2–3× larger). The S02-A09 snap-spike layer plus the locked seed (1337) produces deterministic 1.159 m drift in 1.5 s of flight time. New `tests/unit/test_knuckle_drift.gd` measures + locks the value inside [0.5, 2.0] m (regression boundary, wider than the sweet band so small tuning passes don't break it) AND asserts seed determinism (same launch → identical drift). User pushback after visual test: "la fisica della knuckleball non mi convince" → S05-A04 rewrites the model |
| S05-A08 | Per-zone wet surfaces via `SurfaceZone` Area3D (closes the Sprint 3 deferral) | The Sprint 3 implementation used a single global `config.surface_wet` boolean toggled with the W key. Sprint 5 T05 replaces it with an authorable Area3D: drop a `SurfaceZone` anywhere on the field, set `wet = true`, give it a CollisionShape3D bounding the patch, and the ball flips to wet-surface coefficients (μ_s_wet, rolling_friction_wet, restitution_base_wet, grass_kick_wet) while it overlaps the zone. Implementation: SurfaceZone listens to `body_entered` / `body_exited`, calls `BallPhysics.enter_wet_zone` / `exit_wet_zone` which maintain a refcount (`_wet_zone_count`). New private `_is_wet()` returns `_wet_zone_count > 0 OR config.surface_wet` — the zone overrides, the flag is a fallback so existing W-key behaviour + every Sprint 3 test still pass without modification. Sandbox demo: 5 × 5 m blue translucent patch at world (0, 0, 8). Three new GUT tests lock the override semantics, the refcount stack, and the fallback path. Multi-zone-safe — overlapping wet patches won't flicker the ball state at their boundary |
| S05-A07 | Lock rasoterra trajectories — no tuning needed | T04 `tests/unit/test_rasoterra_levels.gd` measures max bottom-above-ground across 3 s of motion for three power levels along +X with topspin 4 rad/s. Results with current calibration (post-S05-A02 tangential dissipation): strong (30 m/s @ 1°) = 0.012 m max skip, medium (15 m/s @ 3°) = 0.001 m, low (10 m/s @ 1°) = 0.000 m. All three well below the 6 cm regression cap and the 4 cm user target. The angle-aware restitution (S02-A10) correctly recognises these grazing impacts as soft contacts (impact_speed < 0.8) so the bounce path is never entered — the ball transitions to rolling immediately and rolling friction + grass roughness handle the rest. Grass roughness `min_speed = 5 m/s` gate prevents tuft kicks below that threshold (= the low-power case), which is what kept the user-reported "ball flying not rolling" from happening at low power. Closes the Sprint 2 [PENDING] low-power rasoterra item without parameter changes |
| S05-A06 | Knuckle macro launch shape: 30 m/s @ 6° (low flat trajectory) | After S05-A04 the knuckle still felt like a small lob. User clarified the intent: "sale, fluttua a mezz'aria, scende" — Ronaldo / Pirlo low free-kick visual. Changed `BallLauncher.launch_knuckle` from 28 m/s @ 10° to **30 m/s @ 6°**: starting velocity sits inside the drag-crisis band (14-24 m/s with `_effective_drag_coeff`) for ~70 % of flight time so the ball appears to "float" with little deceleration; when it exits the crisis the Cd rises back to 0.47 and the vertical-down knuckle flip bias produces the late dip. Apex 1.22 m (numerically similar to before but qualitatively flatter — vy/vx ratio 0.105 vs 0.176). Final-feel tuning of the stochastic Cy parameters left for Phase 2 when player-controlled shot height lands; user note: "lo revisioneremo quando possibile". Drift with stall+flip + arcade_multiplier 1.5 = 5.99 m / 1.5 s |
| S05-A05 | Knuckle is a per-shot opt-in "special skill", not auto-triggered by speed / spin gates | User observed (visual): the LMB lob was drifting sideways because the gates `|v| > 15 ∧ \|ω\| < 2` happened to fire for a slow spinless lob — but in the design knuckle is a player ability that only certain footballers have, not a generic high-speed-low-spin behaviour. Added `_knuckle_active_for_shot` flag on `BallPhysics`. `reset_knuckle_clock` (already called from every launcher path) clears it; only `BallLauncher.launch_knuckle` calls `set_knuckle_active(true)` after staging the velocity. `knuckle_acceleration` early-returns when the flag is false. New regression test `test_knuckle_off_by_default_special_skill` locks the gate; the existing drift + determinism tests now arm the flag explicitly. 24/24 PASS |
| S05-A09 | **Wet pitch model flipped — wet means "muddy / sticky", supersedes S03-A10** | Sprint 5 visual playtest of T05 surfaced the issue: rolling a rasoterra-debole through the wet patch was barely perceptible. S03-A10 had wet = LOWER friction (rolling 0.10 vs dry 0.30, μ_s 0.15 vs 0.40) — wet pitches "glided" longer. The 5×5 m patch at 10 m/s gave only ~1 m/s speed delta over 0.5 s of contact, well below the perceptual threshold. User pushback: "la decelerazione dovrebbe essere un po' più evidente". Two issues compound: (i) the physical model is debatable — for a soccer ball on grass, a wet pitch with waterlogged sods plows MORE than a dry one (water drag on grass blades, mud grip on the ball envelope). (ii) every football-game player intuition associates wet with stickier / heavier ball, not glidier. New defaults: `rolling_friction_wet = 0.85` (vs dry 0.30), `bounce_mu_s_wet = 0.65` (vs dry 0.40), `restitution_base_wet = 0.35` (mushy bounce, lowered slightly), `grass_roughness_kick_wet = 0.20` (kept — water dampens grass-blade snap). Patch enlarged 5 × 5 → 10 × 10 m so the rolling delta accumulates over more contact time. `test_wet_surface_reduces_friction` / `test_wet_surface_reduces_rolling_friction` renamed to `*_increases_*` and assertions flipped (`assert_lt` → `assert_gt`). Restitution + grass-kick wet assertions kept (still lower wet) |
| S05-A04 | **Stall+flip knuckleball model + drag-crisis Cd(v)** (rewrite, supersedes A03 numerics) | Online research (PMC3660809, PMC9182928, Hong & Asai 2010, Texier 2016, Carré 2004, PMC3657093, Asai 2007) showed our previous SIMPLEX-noise model produced "tremolio" — continuous 1.2 Hz + 5.4 Hz spike noise — while a real knuckleball flips lateral direction only 1–2 times per ~1.5 s flight, holds each direction for 0.35–0.70 s, transitions in ~0.12 s, and operates with `\|Cy\| ≈ 0.05–0.15` (physical) at the contact-time scale. Also discovered: a real soccer ball goes through a **drag crisis** between Re ≈ 2.2–3.3 × 10⁵ (~14–24 m/s) where Cd drops from 0.47 to ~0.18 — the visual signature users recognise from match footage as the ball "not slowing down" through the critical zone. Rewrite: (i) replaced `knuckle_acceleration` with a stall+flip state machine driven by a seeded `RandomNumberGenerator` (full determinism per shot), Cy ramping with `move_toward` at `knuckle_transient_rate = 8 /s` (~0.12 s convergence), axis chosen 70 % lateral / 30 % vertical with 3:1 downward bias for the "late dip" effect. (ii) Added `_effective_drag_coeff(speed)` with smoothstep lerp from `drag_coeff` (subcritical) to `drag_crisis_cd_low = 0.18` across `[drag_crisis_v_low, drag_crisis_v_high] = [14, 24]` m/s. `_drag_force` now reads the effective Cd instead of the constant. (iii) Exposed `knuckle_arcade_multiplier` to scale Cy_effective without rewriting the physics — default **1.5** (option-C arcade band, drift ~5 m at 28 m/s @ 10°; physical 1.0 produces ~3.5 m, heavy arcade 2.0 produces ~8 m). Provisional regression boundary in `test_knuckle_drift.gd` widened to [0.5, 10.0] m while the user hand-tunes via the F1 debug UI; sweet band moved to [2.5, 4.0] m (reported, not asserted). `test_drag_terminal_velocity` (Sprint 1) updated to disable drag crisis since the closed-form terminal velocity formula assumes constant Cd. `test_knuckle_acceleration_deterministic` (Sprint 2) updated: determinism is now at the SEQUENCE level (same seed + same call sequence → identical trajectory), not single-call (every call advances the stall timer). Legacy `knuckle_amplitude` / `noise_frequency` / `spike_*` params remain in PhysicsConfig as `[LEGACY]` for backwards compat — unused by the new model |
...
| 2026-05-12 | T02b    | S05-A01 reopened after user pushback. Literature review (Cross 2002/2010/2014, Carré 2004, Psyonix GDC 2018) showed missing tangential dissipation. S05-A02 adds Cross-2014 `e_t_surface` (0.25) parallel-to-Coulomb and a retention clamp lerp(0.60, 0.85). Test `test_cross_topspin_keeps_or_gains_forward_speed` rewritten qualitatively. 21/21 PASS. Visual lob retest validated by user: "mi pare molto migliorato! per me è PASS". |
| 2026-05-12 | T03     | Knuckle drift measured 1.159 m / 1.5 s — inside old sweet band [0.80, 1.50 m]. No parameter change required initially. New `test_knuckle_drift.gd` locks the value and seed determinism. |
| 2026-05-12 | T03b    | Visual test failed: user reports knuckle "non convince". Online research (Cross / Asai / Mehta / Mizota / Carré / Texier / PMC papers) showed our continuous-noise model is qualitatively wrong — real knuckle is 1-2 discrete flips per 1.5 s with 0.35-0.70 s stalls + ramp, plus a drag crisis at 14-24 m/s. S05-A04 rewrites `knuckle_acceleration` as a stall+flip state machine and adds `_effective_drag_coeff(v)`. New default `knuckle_arcade_multiplier = 1.5` (option C) measures 5.251 m drift — outside sweet [2.5, 4.0] but inside provisional regression boundary [0.5, 10.0]. User to hand-tune via F1 debug UI then lock final parameters. 23/23 PASS. |
| 2026-05-12 | T03c    | User-observed bug: LMB lob drifting sideways because knuckle was auto-triggered by the speed/spin gates. S05-A05 adds an explicit `_knuckle_active_for_shot` flag — only `BallLauncher.launch_knuckle` arms it; every other launcher clears it via `reset_knuckle_clock`. 24/24 PASS. |
| 2026-05-12 | T03d    | User: "sale, fluttua a mezz'aria, scende — la knuckleball ideale non ha una parabola alta". Changed macro from 28 m/s @ 10° to **30 m/s @ 6°** (S05-A06) — apex 1.22 m, drift 5.99 m. User PASS visually; final stochastic-parameter lock deferred to Phase 2 shot-height phase. |
| 2026-05-12 | T04     | Rasoterra triple-power test PASS without tuning (S05-A07). strong 1.2 cm, medium 0.1 cm, low 0.0 cm max skip — all well under 6 cm cap. Closes the Sprint 2 PENDING low-power rasoterra item. 27/27 PASS. |
| 2026-05-12 | T05     | Per-zone surfaces (S05-A08). `SurfaceZone` Area3D + refcount API on `BallPhysics`. Sandbox demo wet patch at (0, 0, 8). 30/30 PASS. Closes the Sprint 3 per-zone deferral. |
| 2026-05-13 | T05/T06 hotfix | Visual playtest report: (a) LMB long lobs overshoot the click — vacuum solver underestimates drag on long shots. First fix attempt (iterative `predict_forward` refinement) regressed worse (~2× overshoot) because `_knuckle_active_for_shot` is sticky and the prediction injected stall-flip force into the simulated trajectory; second fix saves/restores the flag around the prediction call. Headless trace converges to <1 % click error in 2-3 iters at 20 m and 40 m. (b) Wet patch effect imperceptible on rasoterra → S05-A09 flips the pitch model (wet = sticky, not glidey), enlarges the patch to 10 × 10 m. (c) Exposed rasoterra medio (KEY_5, 15 m/s @ 3°) and debole (KEY_6, 10 m/s @ 1°) launchers — only the strong variant was wired before, but it skips across the patch with too little contact for friction to act. 30/30 PASS after the test rename. |
| 2026-05-13 | T05/T06 hotfix-2 | Diagnostic console prints + headless replay of the user's exact lob (v=(-15.64, 10.85, -30.42)) revealed predict converges to 51.80 m, live Godot RigidBody3D lands at 103.58 m — **2.000× exactly**. Smoking gun: with `custom_integrator = true` Godot 4 still applies one `transform.origin += linear_velocity · dt` per tick after `_integrate_forces` returns. Our substep loop already writes the integrated p_final, so each tick double-counts a `v · dt` motion. Bug latent since Sprint 1 — pure-function GUT tests never went through `PhysicsDirectBodyState3D`. Fix: at end of `_integrate_forces`, after the replay buffer record, subtract `linear_velocity · dt` from `state.transform.origin` so Godot's auto-step lands us back on p_final. **All distance-locked numerics from Sprints 1-4 were latently calibrated against 2× travel; recheck on any future tuning.** |
| 2026-05-13 | T05/T06 hotfix-3 | User retest after the 2× fix: lob lands on the click but the parabola is too flat ("teso") and rasoterra forte only reaches ~60 % of the previous range. Both stem from the same root: macros / arc formula were originally calibrated against 2×-travel live behaviour. Bumped lob arc from `clampf(dist · 0.25, 0.5, 6.0)` → `clampf(dist · 0.40, 1.5, 12.0)` (apex up to 12 m on long shots, 8 m on medium) — restores football-cross look, leaves "low pinpoint" pass to a future Phase-2 skill control. Bumped `launch_grounder_topspin` 30 m/s → 40 m/s, the speed the user originally calibrated visually before the 2× bug was identified. `test_rasoterra_levels.test_rasoterra_strong` updated to the new speed; pure-function skip stays well under the 6 cm cap. |
