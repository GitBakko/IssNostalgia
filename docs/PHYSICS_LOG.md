...
| S01-A11 | `debug_visual_scale` on `BallPhysics`, default 2.0 in the sandbox | At the chosen camera (0, 20, 40) the real 0.11 m ball is ~7 px wide — too small to read the spin. Scale 4.0 was tried and rejected as out of proportion vs. the field (a 44 cm ball next to FIFA-regulation 105×68 m lines reads as a beach ball). Scale 2.0 (22 cm rendered, ~14 px on screen) keeps the spin visible while staying in scale with the field. The `CollisionShape3D` and every physics formula keep the FIFA 0.11 m radius. Sandbox-only; set to 1.0 once a near-camera follow system exists |
